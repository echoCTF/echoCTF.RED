...
class PlayerController extends Controller
{
  ...
  */
  public function actionMail($active = false, $email = false, $status = 9, $approval = 1, $sleep = 0, $limit = 40)
  {
    ...
    if ($email !== false && $email != '0') {
      ...
      $players = Player::find()->where(['active' => $active, 'status' => $status, 'approval' => $approval])->limit($limit)->all();
  ...
  public function actionFetchIdentification($filter = 'inactive', $scheme = 'https')
  {
    ...
    foreach ($players->all() as $player) {
      ...
      if ($format !== null && $skip !== true) {
        ...
        echo "$scheme://" . Yii::$app->sys->offense_domain . '/identificationFiles/' . $player->profile->id . $format, "\n";
