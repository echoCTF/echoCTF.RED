...
class PlayerController extends Controller
{
  ...
  */
  public function actionMail($active = false, $email = false, $status = 9, $approval = 1, $sleep = 0, $limit = 0)
  {
    ...
    if ($email !== false && $email != '0') {
      ...
      $q = Player::find()->where(['active' => $active, 'status' => $status, 'approval' => $approval]);
      if (intval($limit) !== 0)
        $q->limit($limit);
      $players = $q->all();
    ...
    foreach ($players as $player) {
      ...
      if ($player->mail($subject, $contentHtml, $contentTxt) === true) {
        if (\Yii::$app->sys->require_approval && $player->approval != 2) {
          $player->approval = 2;
          $player->save();
        }
        $this->stdout(" Ok\n");
  ...
  */
  public function actionRegister($username, $email, $fullname, $password = false, $player_type = "offense", $active = false, $academic = false, $team_name = false, $approved = 0)
  {
    ...
    try {
      ...
      if ($username === '') {
        $unique = false;
        while ($unique !== null) {
          $username = \app\components\Produce::dockernames();
      ...
      if ($password === "")
        $password = $player->genPassword(false);
  ...
  /**
   * Regenerate SSL certificates for players
   */
  public function actionRegenerateCerts($player_id = null)
  {
    if (intval($player_id) === 0)
      $players = Player::find()->where(['active' => 1, 'status' => 10])->all();
    else
      $players = Player::find()->where(['id' => intval($player_id)])->all();
  ...
  public function actionFetchIdentification($filter = 'inactive', $scheme = 'https')
  {
    ...
    switch ($filter) {
      case 'active':
        $players->where(['status' => 10]);
        break;

      case 'inactive':
        $players->where(['status' => 9]);
        break;
    }
    foreach ($players->all() as $player) {
      ...
      if (file_exists($baseDir . $player->metadata->identificationFile)) {
        echo " found local file\n";
      } else {
        $ch = curl_init("$scheme://" . Yii::$app->sys->offense_domain . '/identificationFiles/' . $player->metadata->identificationFile);
