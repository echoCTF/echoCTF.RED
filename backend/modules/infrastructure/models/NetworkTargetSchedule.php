...
class NetworkTargetSchedule extends \yii\db\ActiveRecord
{
  ...
  public function addNews()
  {
    ...
    {
      $bodyPlain=sprintf(\Yii::t('app',"Hi @everyone, just a heads up, the target [**%s**] => https://%s/target/%d is now available for you to headshot on the general targets listing.\n\nHave fun and Happy Hacking :heart:"),$this->target->name,Yii::$app->sys->offense_domain,$this->target_id);
    ...
    {
      $bodyPlain=sprintf(\Yii::t('app',"Hi @everyone, just a heads up, the target [**%s**] => https://%s/target/%d got migrated to the network [**%s**] and its waiting for your headshot.\n\nHave fun and Happy Hacking :heart:"),$this->target->name,Yii::$app->sys->offense_domain,$this->target_id,$this->network->name);
