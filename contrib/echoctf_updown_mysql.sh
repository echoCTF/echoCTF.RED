...
if [ "$script_type" == "client-connect" ]; then
    ...
    USER_LOGGEDIN=$(echo "get ovpn:$common_name"|nc -N ${MEMD} 11211 |egrep -v "(VALUE|END)")
    echo "USER_LOGGEDIN: $USER_LOGGEDIN" >>/tmp/updown.log
    if [ "$USER_LOGGEDIN" == "" ]; then
      echo "[$$] logging in client ${common_name}" >>/tmp/updown.log
      mysql -h 127.0.0.1 echoCTF -e "CALL VPN_LOGIN(${common_name},INET_ATON('${ifconfig_pool_remote_ip}'),INET_ATON('${untrusted_ip}'))"
  ...
  mysql -h 127.0.0.1 echoCTF -e "CALL VPN_LOGOUT(${common_name},INET_ATON('${ifconfig_pool_remote_ip}'),INET_ATON('${untrusted_ip}'))"
