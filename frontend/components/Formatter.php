...
class Formatter extends \yii\i18n\Formatter
{
  ...
  {
    if ($value === null || ($model = \app\models\Player::findOne(['username' => $value])) === null) {
      return $this->nullDisplay;
    }
    return Html::a($model->username, ['/profile/view', 'id' => $model->profile->id], ['class' => 'profile-link', 'title' => \Yii::t('app', 'Go to the profile of [{username}]', ['username' => $model->username])]);
  }
  ...
  {
    if (array_key_exists($value, \app\models\Player::STATUSES))
      return \app\models\Player::STATUSES[$value];
    return $value;
  }
}
