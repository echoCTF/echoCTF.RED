    ...
    <div class="logo">
        ...
        <?php if(!Yii::$app->user->isGuest):?>
          <a href="<?=Url::to(['/profile/me'])?>" class="simple-text logo-normal" style="text-transform:none" title="Profile of <?=Html::encode(Yii::$app->user->identity->username)?>">
            ...
            <?php if(Yii::$app->user->identity->isVip):?><span class="badge badge-danger" style="position: absolute; bottom: 32%; left: 57%">VIP</span><?php endif;?><img style="width: 75px; height: 75px" src="/images/avatars/<?=Yii::$app->user->identity->profile->avtr;?>?<?=time()?>" class="img-fluid rounded border <?php if(Yii::$app->user->identity->isVip):?>border-danger<?php endif;?> <?=\app\components\formatters\RankFormatter::ordinalPlaceCss(Yii::$app->user->identity->profile->rank->id)?>" style="max-width: 60px; max-height: 60px" alt="Avatar of <?=Html::encode(Yii::$app->user->identity->username)?>"><br/>
