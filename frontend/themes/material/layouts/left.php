        ...
        <?php if(!Yii::$app->user->isGuest):?>
          <a href="<?=Url::to(['/profile/me'])?>" class="simple-text logo-normal" style="text-transform:none" title="<?=\Yii::t('app','Profile of')?> <?=Html::encode(Yii::$app->user->identity->username)?><?= Yii::$app->user->identity->onVPN ? "\nVPN IP: ".Yii::$app->user->identity->vpnIP : "" ?>">
            <?php if(Yii::$app->user->identity->isVip):?>
                <span class="badge badge-danger" style="position: absolute; bottom: 32%; left: 57%"><?php if(Yii::$app->user->identity->subscription!==null):?><img src="/images/<?=Yii::$app->user->identity->subscription->product->shortcode?>.svg" width="20px"><?php else:?>VIP<?php endif;?></span>
            <?php endif;?>
          ...
          <center id="clock" class="small clock text-primary"><?=\Yii::t('app','Server time:')?> <span id="time"><?=date('H:i');?> <small><?=date_default_timezone_get()?></small></span></center>
        <?php endif;?>
    </div>
