...
<div class="row">
  ...
  <div class="col-xl-4 col-lg-2 col-md-2 col-sm-12 text-center">
    ...
    <?php
    if ($headshot && $headshot->rating >= 0) {
      echo "<center class='text-primary'>headshoter rating: ", $headshot->rated, "</center>";
    }
  ...
  <div class="col">
    ...
    <?php if (!Yii::$app->user->isGuest && Yii::$app->user->id === $identity->player_id && (Yii::$app->user->identity->getFindings($target->id)->count() > 0 || Yii::$app->user->identity->getTreasures($target->id)->count() > 0)): ?>
      <div class="row">
        ...
        <?php if ($target->spinHistories): ?>
          <div class="col">
            <?= $this->render('_restart_log', ['target' => $target]); ?>
          </div>
        <?php endif; ?>
      </div>
    <?php endif; ?>
