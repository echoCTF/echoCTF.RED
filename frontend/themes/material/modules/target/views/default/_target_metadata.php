<?php if(!Yii::$app->user->isGuest && $metadata):?>
  ...
  <?php if((($identity->player_id===Yii::$app->user->id && $target->progress==100) || Yii::$app->user->identity->isAdmin) && !empty($metadata->post_credits)):?><b><?=\Yii::t('app','Post exploitation credits')?></b>: <?=\yii\helpers\Markdown::process($metadata->post_credits,'gfm')?><br/><?php endif;?>
<?php endif;?>
