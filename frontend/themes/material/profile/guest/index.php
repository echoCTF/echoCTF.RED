...
$this->_fluid="-fluid";
$this->title=Yii::$app->sys->event_name.' '.\Yii::t('app','Profile of: {username}',['username'=>$profile->owner->username]);
...
<div class="profile-index">
  <div class="body-content">
    ...
    <div class="row d-flex justify-content-center">
<?php if($profile->pending_progress):?>
      <div class="col-lg-9">
        <?php
...
<?php if(\Yii::$app->sys->team_only_leaderboards!==true):?>
    <div class="row">
      <div class="col">
        ...
        echo Leaderboard::widget(['divID'=>"Leaderboard", 'player_id'=>$profile->player_id, 'pageSize'=>10, 'title'=>\Yii::t('app','Global Leaderboard'), 'category'=>\Yii::t('app','Listing current player page on a global scale. <small>Updated every 10 minutes</small>'),'pagination'=>false]);
        Pjax::end();
        ?>
      </div>
      <div class="col">
        ...
        echo Leaderboard::widget(['divID'=>"CountryLeaderboard", 'country'=>$profile->country,'player_id'=>$profile->player_id, 'pageSize'=>10, 'title'=>\Yii::t('app','Leaderboard for {country_name}',['country_name'=>$profile->fullCountry->name]), 'category'=>\Yii::t('app','Listing current player page for {country_name}. <small>Updated every 10 minutes</small>',['country_name'=>$profile->fullCountry->name]),'pagerID'=>'country-leaderboard-pager','pagination'=>false]);
        Pjax::end();
        ?>
      </div>
    </div>
    <div class="row">
      ...
      echo Stream::widget(['divID'=>'stream', 'dataProvider' => null, 'player_id'=>$profile->player_id, 'pagerID'=>'stream-pager','title'=>'User History','category'=>'User history on the platform','pagination'=>false,'guest'=>\Yii::$app->user->isGuest]);
      \yii\widgets\Pjax::end();
      ?></div>
    </div><!--/row-->
<?php endif; ?>
