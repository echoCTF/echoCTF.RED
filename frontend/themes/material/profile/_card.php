...
<!--    <h6 class="card-category text-gray"><?=$profile->rank->ordinalPlace?> Place / Level <?=$profile->experience->id?> (<?=$profile->experience->name?>) <img src="/images/flags/shiny/24/<?=$profile->country?>.png"/></h6>-->
  ...
  <?php if(intval(Yii::$app->user->id)===intval($profile->player_id) || Yii::$app->user->identity->isAdmin):?>
  ...
  <?php endif;?>
          ...
          <?php if (trim($profile->twitter)):?><li class="nav-item text-left"><strong><i class="fab fa-twitter"></i> Twitter</strong> <span class="pull-right"><?=Html::a('@'.Html::encode($profile->twitter),"https://twitter.com/".Html::encode($profile->twitter),['target'=>'_blank'])?></span></li><?php endif;?>
