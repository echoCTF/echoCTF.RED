...
class Stream extends StreamAR
{
  ...
  public function getAbuseMessage()
  {
    if ($this->player->teamPlayer) {
      $prefix = sprintf(
        "<img src='/images/avatars/team/%d.png' class='rounded' width='25px'> <b><code>%s</code></b> %s",
        $this->player->team->id,
        Html::encode($this->player->team->name),
        $this->icon
      );
    } else {
      $prefix = sprintf(
        "<img src='/images/avatars/%d.png' class='rounded' width='25px'> <b><code>%s</code></b> %s",
        $this->player_id,
