...
class Target extends TargetAR
{
   ...
   */
  public function getCountHeadshots(): int
  {
    return intval($this->getHeadshots()->count());
  }
  ...
  {
    return !$this->spinDenied;
  }

  /**
   * Check if spin should be denied for the target
   * @returns bool
   */
  public function getSpinDenied(): bool
  {
    // target not spinnable by player
    if ($this->player_spin === false) {
      return true;
    }

    // target not active or already queued for power up
    if ($this->spinQueue !== null || $this->active === false) {
      return true;
    }

    // target status is not online
    if ($this->status !== 'online')
      return true;

    // player is not connected to the VPN
    if (!Yii::$app->user->identity->onVPN)
      return true;
